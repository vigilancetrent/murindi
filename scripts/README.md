...
补齐 `Murindi-Vue-src/src/locales/lang/*.js` 多语言文件缺失 key 的工具链。
...
```bash
# 单个文件回滚
copy Murindi-Vue-src\src\locales\lang\ja-JP.js.bak Murindi-Vue-src\src\locales\lang\ja-JP.js

# 全部回滚
for %f in (Murindi-Vue-src\src\locales\lang\*.bak) do copy "%f" "%~dpnf"
```
...
```bash
# 删除所有 .bak
del Murindi-Vue-src\src\locales\lang\*.bak
# 删除缓存
