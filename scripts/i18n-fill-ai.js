...
const LANG_DIR = path.join(__dirname, '..', 'Murindi-Vue-src', 'src', 'locales', 'lang')
...
const PROVIDERS = {
  ...
  openrouter: {
    ...
    headers: (key) => ({
      Authorization: `Bearer ${key}`,
      'content-type': 'application/json',
      'HTTP-Referer': 'https://murindi.local',
      'X-Title': 'Murindi i18n'
    }),
    buildBody: (model, system, user) => ({
      model,
      messages: [
...
// ---------- AI ----------
const SYSTEM_PROMPT = (targetLangName, note) => `You are a professional i18n translator for Murindi — a web application covering crypto/stock charts, technical indicators, backtesting, live trading, and automated trading bots.
...
4. Do NOT translate these proper nouns/terms: BTC, ETH, USDT, USDC, API, OKX, Binance, Bitget, Bybit, Gate, MEXC, HTX, Kraken, TradingView, RSI, MACD, EMA, SMA, VWAP, Bollinger, ATR, Murindi, AI, JSON.
