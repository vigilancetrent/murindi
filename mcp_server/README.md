...
[![PyPI](https://img.shields.io/pypi/v/murindi-mcp?style=flat-square&logo=pypi&logoColor=white)](https://pypi.org/project/murindi-mcp/)
[![Python](https://img.shields.io/pypi/pyversions/murindi-mcp?style=flat-square&logo=python&logoColor=white)](https://pypi.org/project/murindi-mcp/)
[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg?style=flat-square)](../LICENSE)

Thin Model Context Protocol server that exposes a curated subset of the
Murindi Agent Gateway (`/api/agent/v1`) as MCP tools, so AI clients
that support MCP (Cursor, Claude-style desktop apps, OpenClaw, NanoBot, etc.)
can drive Murindi without writing custom HTTP code.
...
|----------|----------|---------|
| `MURINDI_BASE_URL`     | yes | e.g. `http://localhost:8888` |
| `MURINDI_AGENT_TOKEN`  | yes | a token issued via `/api/agent/v1/admin/tokens` |
| `MURINDI_MCP_TRANSPORT`| no  | `stdio` (default), `sse`, or `streamable-http` |
| `MURINDI_MCP_HOST`     | no  | bind host for HTTP transports (default `127.0.0.1`) |
| `MURINDI_MCP_PORT`     | no  | bind port for HTTP transports (default `8000`) |
| `MURINDI_TIMEOUT_S`    | no  | upstream HTTP timeout (default `60`) |
| `MURINDI_MCP_JOB_STREAM_MAX_EVENTS` | no | SSE cap (default `200`) |
| `MURINDI_MCP_JOB_STREAM_MAX_SECONDS` | no | SSE time cap (default `300`) |
| `MURINDI_MCP_JOB_POLL_MAX_SECONDS` | no | `wait_for_job` cap (default `300`) |

### stdio (desktop IDEs)

```bash
MURINDI_BASE_URL=http://localhost:8888 \
MURINDI_AGENT_TOKEN=qd_agent_xxxxx \
murindi-mcp
```
