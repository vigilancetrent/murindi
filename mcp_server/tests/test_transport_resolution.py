...
def fresh_module(monkeypatch):
    ...
    monkeypatch.setenv("MURINDI_BASE_URL", "http://localhost:8888")
    monkeypatch.setenv("MURINDI_AGENT_TOKEN", "qd_agent_test_token")
    sys.modules.pop("murindi_mcp.server", None)
    ...
    return importlib.import_module("murindi_mcp.server")


def test_transport_default_is_stdio(monkeypatch, fresh_module):
    monkeypatch.delenv("MURINDI_MCP_TRANSPORT", raising=False)
...
def test_apply_http_settings_without_error(monkeypatch, fresh_module):
    ...
    monkeypatch.setenv("MURINDI_MCP_HOST", "0.0.0.0")
    monkeypatch.setenv("MURINDI_MCP_PORT", "7777")
