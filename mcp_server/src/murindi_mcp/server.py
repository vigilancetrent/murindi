...
Murindi MCP server — exposes the Agent Gateway as MCP tools.
...
def _env(name: str, required: bool = True) -> str:
    ...
    if not value and required:
        ...
        print(
            ...
            f"[murindi-mcp] missing required env var: {name}",
...
BASE_URL = _env("MURINDI_BASE_URL").rstrip("/")

AGENT_TOKEN = _env("MURINDI_AGENT_TOKEN")

TIMEOUT_S = float(os.environ.get("MURINDI_TIMEOUT_S", "60"))

JOB_STREAM_MAX_EVENTS = int(os.environ.get("MURINDI_MCP_JOB_STREAM_MAX_EVENTS", "200"))

JOB_STREAM_MAX_SECONDS = float(os.environ.get("MURINDI_MCP_JOB_STREAM_MAX_SECONDS", "300"))

JOB_POLL_MAX_SECONDS = float(os.environ.get("MURINDI_MCP_JOB_POLL_MAX_SECONDS", "300"))
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Tools for the Murindi self-hosted quant platform. "
...
def get_indicator_authoring_contract() -> Any:
    ...
    """Fetch Murindi indicator I/O contract + starter Python template.
...
def _resolve_transport() -> str:
    ...
    raw = (os.environ.get("MURINDI_MCP_TRANSPORT") or "stdio").strip().lower()
    ...
    if raw not in _TRANSPORTS:
        ...
        print(
            ...
            f"[murindi-mcp] unknown transport '{raw}'. "
...
def _apply_http_settings_from_env() -> None:
    ...
    host = (os.environ.get("MURINDI_MCP_HOST") or "").strip()

    port_raw = (os.environ.get("MURINDI_MCP_PORT") or "").strip()
    ...
    if port_raw:
        ...
        try:
            ...
            print(
                ...
                f"[murindi-mcp] invalid MURINDI_MCP_PORT='{port_raw}', ignoring.",
...
def main() -> None:
    ...
    Transport selection (env-only — works in both desktop and cloud):
      ...
      MURINDI_MCP_TRANSPORT=stdio              (default; stdin/stdout)

      MURINDI_MCP_TRANSPORT=sse                (SSE over HTTP)

      MURINDI_MCP_TRANSPORT=streamable-http    (newer MCP HTTP transport)

      MURINDI_MCP_HOST=0.0.0.0                 (bind for HTTP transports)

      MURINDI_MCP_PORT=7800                    (port for HTTP transports)
