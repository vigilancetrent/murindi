#!/bin/bash
#
# Murindi One-Click Installation Script
# https://github.com/vigilancetrent/Murindi
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/vigilancetrent/Murindi/main/install.sh | bash
#
# Custom install directory:
#   curl -fsSL https://raw.githubusercontent.com/vigilancetrent/Murindi/main/install.sh | bash -s -- /opt/murindi
#
...
INSTALL_DIR="${1:-$HOME/murindi}"
COMPOSE_SOURCE="docker-compose.ghcr.yml"
GITHUB_RAW="https://raw.githubusercontent.com/vigilancetrent/Murindi/main"
FRONTEND_PORT="${FRONTEND_PORT:-8888}"
BACKEND_PORT="${BACKEND_PORT:-5000}"

echo -e "${BLUE}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║           Murindi — AI Quant Operating System          ║"
...
start_services() {
    echo -e "${YELLOW}Starting Murindi services...${NC}"
    $COMPOSE_CMD up -d
...
print_success() {
    ...
    echo ""
    echo -e "  ${BLUE}Default login:${NC}  murindi / 123456"
    ...
    echo -e "╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${GREEN}curl -fsSL https://raw.githubusercontent.com/vigilancetrent/Murindi/main/install.sh | bash${NC}"
    echo ""
