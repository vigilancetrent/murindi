# Agent Quickstart — using Murindi from an AI agent

This quickstart shows how to drive the Murindi Agent Gateway
...
Two transports are supported via `MURINDI_MCP_TRANSPORT`:
...
* `sse` / `streamable-http` — cloud agents and remote IDEs that connect to a
  long-running HTTP endpoint. Combine with `MURINDI_MCP_HOST` /
  `MURINDI_MCP_PORT`.
