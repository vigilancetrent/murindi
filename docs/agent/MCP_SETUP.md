# MCP Setup — connect Cursor / Claude Code / Codex to Murindi

This is the detailed setup guide for wiring an MCP-capable AI client (Cursor,
Claude Code, Codex desktop, OpenClaw, NanoBot, …) to a Murindi backend.
The root [`README`](../../README.md) gives the 30-second pitch; everything
below is the actual recipe.

The Murindi backend exposes an **Agent Gateway** at `/api/agent/v1`, and a
small MCP server (published on PyPI as
[`murindi-mcp`](https://pypi.org/project/murindi-mcp/)) wraps that
...
value of `MURINDI_BASE_URL` changes.

### Path A · Hosted ([ai.murindi.com](https://ai.murindi.com)) — 30 seconds

Best for: trying Murindi from Cursor / Claude Code without installing
anything; demos; research notebooks against shared datasets.

1. Sign up at [ai.murindi.com](https://ai.murindi.com).
2. Open **Profile → My Agent Token** → **Issue Token**.
3. `MURINDI_BASE_URL=https://ai.murindi.com`.
...
3. `MURINDI_BASE_URL=http://localhost:8888` (or your LAN URL).
...
[`murindi-mcp`](https://pypi.org/project/murindi-mcp/). Drop this into
...
```json
{
  "mcpServers": {
    "murindi": {
      ...
      "args": ["murindi-mcp"],
      "env": {
        "MURINDI_BASE_URL":    "http://localhost:8888",
...
```bash
pip install murindi-mcp
# then use {"command": "murindi-mcp", "args": []}
```

For Claude Code's CLI helper:

```bash
claude mcp add murindi \
  --env MURINDI_BASE_URL=http://localhost:8888 \
  --env MURINDI_AGENT_TOKEN=qd_agent_xxxxxxxx \
  -- uvx murindi-mcp
```
...
```bash
MURINDI_BASE_URL=https://your-host \
MURINDI_AGENT_TOKEN=qd_agent_xxxxxxxx \
MURINDI_MCP_TRANSPORT=streamable-http \
MURINDI_MCP_HOST=0.0.0.0 \
MURINDI_MCP_PORT=7800 \
murindi-mcp
# clients connect to http://your-host:7800
```

Use `MURINDI_MCP_TRANSPORT=sse` instead for clients that only speak the
...
(capped by `MURINDI_MCP_JOB_STREAM_MAX_*` env vars). Raw Gateway SSE is
...
## Using Murindi as a *coding* agent context

If you're editing this repo with Cursor / Claude Code / Codex, the repo also
ships a Cursor Skill at
[`.cursor/skills/murindi-agent-workflow/SKILL.md`](../../.cursor/skills/murindi-agent-workflow/SKILL.md)
...
- [MCP server README](../../mcp_server/README.md) — installation, env vars,
  and developer notes for the `murindi-mcp` package itself.
