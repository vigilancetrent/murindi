# Murindi Cloud Server Deployment Guide
...
Recommended site config `/etc/nginx/sites-available/murindi.conf`:
...
```bash
sudo ln -s /etc/nginx/sites-available/murindi.conf /etc/nginx/sites-enabled/murindi.conf
sudo nginx -t
...
- The frontend is pulled as a prebuilt image (`ghcr.io/vigilancetrent/murindi-frontend`) — no local build
...
### 4. `docker compose up` fails to pull `murindi-frontend`

The frontend image lives on GHCR (`ghcr.io/vigilancetrent/murindi-frontend`). If the pull fails:

```bash
docker pull ghcr.io/vigilancetrent/murindi-frontend:latest
```
