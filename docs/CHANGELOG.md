...
### Requires
- Murindi backend with Agent Gateway `/api/agent/v1` (indicator routes from this repo)
...
2. **Frontend**: rebuild or run `npm run dev` with current `Murindi-Vue-src`; hard-refresh Profile to see the Login history tab.
...
| Profile UI | `Murindi-Vue-src/src/views/profile/index.vue` |
...
| `Murindi-Vue-src/src/views/billing/index.vue` | Chain selection modal + URI QR + suffix highlight |
| `Murindi-Vue-src/src/api/billing.js` | `listUsdtChains()` / `createUsdtOrder(plan, chain)` |
...
Third **traditional broker** integrated into Murindi, on par with IBKR / MT5 (from [PR #101](https://github.com/vigilancetrent/Murindi/pull/101)):
...
When an exchange lacks a native timeframe (e.g. OKX has no 30m), backend fetches finer OHLCV and exchange-aligned resample. Frontend unchanged; 13 new unit tests (from [PR #104](https://github.com/vigilancetrent/Murindi/pull/104)).
...
`backend_api_python/app/services/oauth_service.py` parses `FRONTEND_URL` as comma-separated list: first entry = default post-login redirect origin; all entries = OAuth redirect whitelist. One backend can serve `ai.murindi.com` + `m.murindi.com`.
...
- **Frontend**: build `Murindi-Vue-src` and replace `frontend/dist` (repo `frontend/dist` includes this build)
...
#### MCP Server (`mcp_server/` — published on PyPI: [`murindi-mcp`](https://pypi.org/project/murindi-mcp/))
Standalone Python package wrapping Agent Gateway R / B subset as Model Context Protocol tools:
- One-line install (any machine, no repo clone): `uvx murindi-mcp` / `pipx install murindi-mcp` / `pip install murindi-mcp`
- Three transports via `MURINDI_MCP_TRANSPORT`:
...
- HTTP mode also reads `MURINDI_MCP_HOST` / `MURINDI_MCP_PORT`
...
- `docs/agent/AI_INTEGRATION_DESIGN.md` — Agent design doc consuming Murindi as a **product** (personas, capability classes, security, roadmap, progress table). Currently v0.3
...
- `.cursor/skills/murindi-agent-workflow/SKILL.md` — Skill for Cursor / Claude Code: red lines, entry points, validation in this repo
...
| `MURINDI_MCP_TRANSPORT` | `stdio` | MCP client transport (`stdio` / `sse` / `streamable-http`) |
| `MURINDI_MCP_HOST` | `127.0.0.1` | MCP HTTP bind host |
| `MURINDI_MCP_PORT` | `8000` | MCP HTTP bind port |
...
```sql
-- ============================================================
-- Murindi V3.1.0 Database Migration
...
DO $$ BEGIN RAISE NOTICE '✅ Murindi V3.1.0 agent gateway schema migration completed!'; END $$;
```

**Docker one-liner example:**

```bash
docker compose exec -T postgres psql -U murindi -d murindi \
...
```bash
docker cp /path/to/v3.1.0_agent_gateway.sql murindi-db:/tmp/migrate.sql
docker compose exec -T postgres psql -U murindi -d murindi -f /tmp/migrate.sql
```
...
- `mcp_server/src/murindi_mcp/{__init__.py, server.py}` — `FastMCP` + `httpx`, three transports via env
- `mcp_server/tests/test_transport_resolution.py`

**Frontend (`Murindi-Vue-src/` + synced to `frontend/dist/`):**
...
- `.cursor/skills/murindi-agent-workflow/SKILL.md`
...
4. **MCP HTTP production**: `streamable-http` binds `127.0.0.1` by default; for external access set `MURINDI_MCP_HOST=0.0.0.0` behind nginx / reverse proxy — **Agent-token clients only**
...
- **`.gitignore`** — ignore `scripts/.i18n-cache/` and `Murindi-Vue-src/src/locales/lang/*.bak`
...
- `Murindi-Vue-src/src/locales/lang/{ar-SA,de-DE,en-US,fr-FR,ja-JP,ko-KR,th-TH,vi-VN,zh-TW}.js`
...
- `Murindi-Vue-src/src/views/trading-bot/components/BotCreateWizard.vue` — martingale/trend forced market orders
- `Murindi-Vue-src/src/views/trading-bot/components/botScriptTemplates.js` — grid dual budget, DCA time-based interval + external close reset
- `Murindi-Vue-src/src/views/trading-bot/components/configs/GridConfig.vue`, `DCAConfig.vue` — param validation
- `Murindi-Vue-src/src/locales/lang/*.js` — 4 new validation strings × 10 languages
...
- **Backtest Buy/Sell markers misaligned on chart**: after indicator IDE backtest, B/S markers could shift one bar (worse in MTF). Two causes:
  ...
  - Files: `backend_api_python/app/services/backtest.py`, `Murindi-Vue-src/src/views/indicator-ide/index.vue`
...
```bash
docker compose exec -T postgres psql -U murindi -d murindi <<'SQL'
...
- `Murindi-Vue-src/package.json`, `src/config/defaultSettings.js`, `src/layouts/BasicLayout.vue` version `3.0.1 → 3.0.2`; `README.md` and `docs/README_CN.md` badges synced
...
```bash
docker cp backend_api_python/migrations/<your>.sql murindi-db:/tmp/migrate.sql
docker compose exec -T postgres psql -U murindi -d murindi -f /tmp/migrate.sql
```
...
- **Docs**: root `README.md` and `docs/README_CN.md` add Murindi exchange referral signup links (same as Profile “Open account”); version badge → 3.0.1.
...
```sql
-- ============================================================
-- Murindi V2.2.4 Database Migration
...
```sql
-- ============================================================
-- Murindi V2.2.3 — qd_users.timezone (user profile timezone)
-- ============================================================
...
```sql
-- ============================================================
-- Murindi V2.2.2 Database Migration
-- Polymarket Prediction Markets Integration
-- ============================================================
...
DO $$
BEGIN
    RAISE NOTICE '✅ Murindi V2.2.2 database migration completed!';
END $$;
```
...
```sql
-- ============================================================
-- Murindi V2.2.1 Database Migration
...
DO $$
BEGIN
    RAISE NOTICE '✅ Murindi V2.2.1 database migration completed!';
END $$;
```
...
```sql
-- ============================================================
-- Murindi V2.1.3 Database Migration
-- Cross-Sectional Strategy Support
-- ============================================================
...
```sql
-- ============================================================
-- Murindi V2.1.1 Database Migration
-- ============================================================
...
DO $$
BEGIN
    RAISE NOTICE '✅ Murindi V2.1.1 database migration completed!';
END $$;
```
