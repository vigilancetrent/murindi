...
本文面向生产/准生产环境，使用云服务器 + Docker Compose 部署 Murindi，并补充域名、HTTPS、反向代理与前后端分离说明。
...
推荐站点配置 `/etc/nginx/sites-available/murindi.conf`：
...
- 前端以预构建镜像（`ghcr.io/vigilancetrent/murindi-frontend`）方式拉取，无需本地构建
...
### 4. `docker compose up` 拉不到 `murindi-frontend`

前端镜像托管在 GHCR（`ghcr.io/vigilancetrent/murindi-frontend`）。若拉取失败：
