...
This guide explains how to configure Murindi for multi-user mode with PostgreSQL database.
...
CREATE DATABASE murindi;
CREATE USER murindi WITH ENCRYPTED PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE murindi TO murindi;
\q
```
...
```bash
# Run init.sql
psql -U murindi -d murindi -f backend_api_python/migrations/init.sql
```
...
DB_TYPE=postgresql
DATABASE_URL=postgresql://murindi:your_password@localhost:5432/murindi
...
export DATABASE_URL=postgresql://murindi:your_password@localhost:5432/murindi
...
# Check connection
psql -U murindi -d murindi -c "SELECT 1"
```
...
```bash
# Check SQLite path
ls -la backend_api_python/data/murindi.db

# Check PostgreSQL tables
psql -U murindi -d murindi -c "\dt"
```
