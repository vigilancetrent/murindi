<div align="center">
  ...
  <div align="center" style="max-width: 680px; margin: 1.25rem auto 0; padding: 20px 22px 22px; border: 1px solid #d1d9e0; border-radius: 16px;">
    ...
    <p style="margin: 0 0 18px; padding-bottom: 16px; border-bottom: 1px solid #eaeef2; line-height: 2;">
      ...
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
      <a href="https://www.murindi.com"><strong>官网</strong></a>
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
    </p>
  ...
  <p style="margin: 12px 0 10px;">
    <a href="https://oosmetrics.com/repo/vigilancetrent/Murindi"><img src="https://api.oosmetrics.com/api/v1/badge/achievement/4991ab54-52d2-46d4-a03a-67b47b61ae4b.svg" alt="oosmetrics — Training 类第 7 名（2026-04-25）"></a>
  </p>
...
[两分钟试用](#两分钟试用) · [为什么选择 Murindi](#为什么选择-murindi) · [安全模型](#安全模型) · [技术亮点](#技术亮点) · [相关仓库](#相关仓库) · [MCP 与 Agent 网关](#mcp-agent-gateway) · [产品概览](#产品概览) · [功能一览](#功能一览) · [视觉导览](#视觉导览) · [架构](#架构) · [安装](#安装与首次运行) · [文档](#文档导航) · [常见问题](#常见问题) · [许可](#许可与商业说明)
...
默认安装到 `~/murindi`（自定义：`… | bash -s -- /opt/murindi`）。重复执行同一命令可拉取最新镜像并重启。

然后打开 **`http://localhost:8888`**，使用 **`murindi` / `123456`** 登录，并**修改默认管理员密码**。

<details>
<summary><b>Windows、手动克隆或镜像加速排错</b></summary>

**Windows（PowerShell）** —— `git clone` 后目录名为 **`Murindi`**：
...
Murindi 是**可自托管、本地优先**的量化基础设施层——不是带买入按钮的聊天机器人。它在同一套生产级栈里统一 **多 LLM 研究**、**Python 原生策略引擎**、**服务端回测** 与 **多券商实盘**（10+ 加密货币 venue、IBKR、MT5、Alpaca），完全由你掌控。

## 安全模型

- **Agent token 默认仅纸面** —— 实盘交易需服务端显式解锁。
- **实盘执行需明确授权** —— token scope + 自托管栈上的 `AGENT_LIVE_TRADING_ENABLED`。
- **交易所密钥留在用户自己的部署内** —— 自托管安装不由 Murindi SaaS 运营方持有。
- **每次 Agent 调用写入审计日志** —— 供自动化与合规审查的 append-only 审计链。
- **Murindi 不提供投资建议** —— 软件仅用于合法的研究与执行；合规与风险由你自行负责。
...
<div align="center">
  <img src="screenshots/ezgif.com-animated-gif-maker.gif" alt="Murindi 快速演示：安装、登录、图表、AI 分析与策略工作流" width="920" style="border-radius: 12px; border: 1px solid #eaeef2;">
  <p><sub><em>从零到跑通——图表、AI 研究与策略工作流，几分钟搞定。</em></sub></p>
</div>

<div align="center">
  <img src="screenshots/architecture.png" alt="Murindi 系统架构：行情数据 → 指标 / 信号 / 策略 / 回测 / AI 分析 → 执行，闭环量化工作流（想法 → 指标 → 策略 → 回测 → 优化 → 执行 → 监控）" width="960">
...
| **Agent 原生** | 一等公民 **Agent Gateway**（`/api/agent/v1`）+ PyPI 上的 **[`murindi-mcp`](https://pypi.org/project/murindi-mcp/)**——Cursor、Claude Code、Codex 可读行情、跑回测、下单（默认纸面），全链路审计。 |
...
| 仓库 | 说明 |
|------|------|
| **[Murindi](https://github.com/vigilancetrent/Murindi)**（本仓库） | 后端（Flask/Python）、Compose 部署栈、文档 |
| **[Murindi-Vue](https://github.com/vigilancetrent/Murindi-Vue)** | **Web 前端源码**（Vue）—— 打 `v*` tag 即自动构建并推送 `ghcr.io/vigilancetrent/murindi-frontend` |
| **[Murindi-Mobile](https://github.com/vigilancetrent/Murindi-Mobile)** | **开源移动端**，连接你自托管或 SaaS 的同一套后端 |

**说明：** 只有想从 **Murindi-Vue** 自行构建 Web 时才需要 Node.js；默认 Docker 快速上手会直接拉取已发布镜像。

<h2 id="mcp-agent-gateway">用 AI Agent 接入（Cursor / Claude Code / Codex / MCP）</h2>

Murindi 自带 **Agent Gateway**（`/api/agent/v1`）和已发布到 PyPI 的轻量 **MCP 服务器**（[`murindi-mcp`](https://pypi.org/project/murindi-mcp/)）。签发一个 token，AI 客户端即可读行情、跑回测、管理策略，并按默认纸面规则下单——**不会接触你的交易所密钥与管理员 JWT**。
...
**两套后端，客户端配置一模一样——只是 `MURINDI_BASE_URL` 不同：**

- **云端（30 秒上手）** —— 在 [ai.murindi.com](https://ai.murindi.com) 注册 → **个人中心 → 我的 Agent Token** → 签发。支持 **T（交易）scope**，**默认仍仅纸面**；实盘需 token 上 `paper_only=false`、签发时勾选风险确认，且服务器 `AGENT_LIVE_TRADING_ENABLED=true`。SaaS 多租户开放 T 会加大共享基础设施负载与平台运营风险，详见页面内风险说明。
...
**栈结构：** Nginx 提供预构建 Vue SPA（`ghcr.io/vigilancetrent/murindi-frontend`）；**Flask + Gunicorn** API 承载策略、AI、计费与 Agent 服务；**PostgreSQL 16** 为系统记录库；**Redis 7** 支撑缓存与 Worker 协调。交易所、经纪商、LLM、支付通过 env 驱动适配器接入——换供应商无需 fork 核心代码。

**运行时流程：** 行情 → 指标/信号层 → 策略引擎 → 回测或实盘运行时 → venue 专属执行适配器；挂单由后台 Worker 派发，带健康检查与重试。

**部署面：** 一行 `install.sh`、零仓库 GHCR Compose、完整仓库 Compose（本地构建后端）、AWS Marketplace AMI，以及 [ai.murindi.com](https://ai.murindi.com) SaaS 试用。
...
下文对应常见「本地部署」顺序：**准备宿主机 → 拉代码 → 配密钥 → 起栈 → 自检 → 加固 → 可选接入大模型**。**不需要 Node.js**：`frontend` 服务直接从 GHCR 拉取 `ghcr.io/vigilancetrent/murindi-frontend` 并由 Nginx 提供，无需本地构建。
...
- **`frontend`**：拉取 `ghcr.io/vigilancetrent/murindi-frontend:latest`，无需本地 Vue 目录。
- **`backend`**：首次若无本地镜像，会从 `./backend_api_python` 自动构建。
- 想用本地 Vue 源码做 UI 开发，请将 **Murindi-Vue** 克隆到 `./Murindi-Vue/`，并在命令中追加 `-f docker-compose.build.yml`（见下文 *从 Vue 源码构建前端*）。
...
# BACKEND_IMAGE=ghcr.io/<你的fork>/murindi-backend     # 可选，用于 fork
# FRONTEND_IMAGE=ghcr.io/<你的fork>/murindi-frontend
```

Tag 解析优先级：`BACKEND_TAG` / `FRONTEND_TAG` → `IMAGE_TAG` → compose 默认值 (`latest`)。无根目录 `.env` 时，两个 compose 文件都拉 `ghcr.io/vigilancetrent/murindi-{backend,frontend}:latest`。想固定到某个版本就设置 `IMAGE_TAG`（前后端一起）或 `BACKEND_TAG` / `FRONTEND_TAG`（单边）—— 可用 tag 见 [GitHub Releases](https://github.com/vigilancetrent/Murindi/releases)。

#### 备选方案：从 Vue 源码本地构建前端

如果你有 **Murindi-Vue** 仓库的访问权限，想改 UI 源码（换主题、二开、调试），把它克隆到本仓根目录下的 `./Murindi-Vue/`（已 gitignore），让 Compose 直接从那里构建：
...
主 `docker-compose.yml` 只声明拉镜像；override 文件 `docker-compose.build.yml` 额外加上本地 `build:` 块。不叠加 override 时，`./Murindi-Vue/` 不需要存在。想换源码路径就设 `FRONTEND_SRC_PATH=/abs/path/to/Murindi-Vue`，或者在根目录 `.env` 里加 `COMPOSE_FILE=docker-compose.yml:docker-compose.build.yml` 省掉长长的 `-f -f` 写法。本地构建出来的镜像 tag 仍走 `FRONTEND_TAG` / `IMAGE_TAG` 那套规则，跟其它服务无缝衔接，不用改其它配置。
...
| 现象 | 排查 |
|------|------|
| `Murindi-Vue` 路径不存在 | 加了 `-f docker-compose.build.yml` 但未克隆 Vue 源码；去掉 override（直接 `docker compose up -d`），或先克隆到 `./Murindi-Vue/`。 |
...
下面这种 Python 风格，就是 Murindi 的典型策略开发方式：
...
| [Agent 环境设计](agent/AGENT_ENVIRONMENT_DESIGN.md) · [AI / Agent 集成](agent/AI_INTEGRATION_DESIGN.md) · [快速开始](agent/AGENT_QUICKSTART.md) · [OpenAPI](agent/agent-openapi.json) · [MCP 说明](../mcp_server/README.md) | 编码 Agent、网关、MCP（PyPI：`murindi-mcp`）；部分正文为英文 |
...
可以。Murindi 同时支持基于数据表的 `IndicatorStrategy` 和事件驱动的 `ScriptStrategy`。你也可以先让 AI 生成初稿，再自己继续修改。

### 它到底是研究工具还是实盘交易平台？

两者都是。Murindi 想打通的是 AI 研究、图表、策略开发、回测、快速交易和实盘运营，而不是只做其中某一段。

### 可以商用吗？

后端为 **Apache 2.0**。**Web 前端源码**（[Murindi-Vue](https://github.com/vigilancetrent/Murindi-Vue)）适用单独的 source-available 条款—商用前请阅读并按需取得前端商业授权。**[移动端仓库](https://github.com/vigilancetrent/Murindi-Mobile)** 单独开源，许可以该仓库为准。

### 有移动端吗？

有。见 **[Murindi-Mobile](https://github.com/vigilancetrent/Murindi-Mobile)**（开源），可连接你自托管或 SaaS 的同一后端。
...
| 交易所 | 注册链接 |
|--------|----------|
| Binance | [注册开户](https://www.bsmkweb.cc/register?ref=MURINDI) |
| Bitget | [注册开户](https://partner.hdmune.cn/bg/7r4xz8kd) |
| Bybit | [注册开户](https://partner.bybit.com/b/DINGER) |
| OKX | [注册开户](https://www.xqmnobxky.com/join/MURINDI) |
...
- 前端源码单独公开在 [Murindi Frontend](https://github.com/vigilancetrent/Murindi-Vue)，并适用 **Murindi Frontend Source-Available License v1.0**。
...
Murindi 仅用于**合法**的研究、教育与合规交易场景；禁止用于欺诈、市场操纵、逃避制裁、洗钱等违法用途。部署与运营须遵守所在地法律法规及交易所规则。**本项目不提供法律、税务或投资建议。** 使用与误用所致损失与合规风险由使用者自行承担；在适用法律允许范围内，贡献者不对交易亏损、服务中断或监管后果承担责任。
...
- [问题反馈 / 功能建议](https://github.com/vigilancetrent/Murindi/issues)
- Email: [support@murindi.com](mailto:support@murindi.com)
...
Murindi 是向物理学家 **[薛定谔（Erwin Schrödinger）](https://zh.wikipedia.org/wiki/%E5%9F%83%E5%B0%94%E6%B8%A9%C2%B7%E8%96%9B%E5%AE%9A%E8%B0%94)** 的一份小小致敬 —— 名字里的 "-dinger"，正是 "Schrödinger" 的尾巴。盒子里的猫是个思想实验；**每一条还没下单的策略**，都是它的小型版本 —— 在被市场执行之前，**既盈利也亏损**。回测打开盒子，实盘塌缩波函数。请谨慎下单。

<p align="center"><sub>如果 Murindi 对你有帮助，欢迎点一个 GitHub Star。</sub></p>
