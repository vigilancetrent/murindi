# Murindi Python API (backend)

Flask-based backend for Murindi: market data, indicators, AI analysis, backtesting, and a strategy runtime with multi-user support.
...
# Initialize schema
psql -U murindi -d murindi -f migrations/init.sql
```
...
`enabled=false` and the rest of Murindi continues to work normally.
