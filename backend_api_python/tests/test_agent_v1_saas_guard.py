...
def test_is_saas_mode_recognizes_known_spellings(monkeypatch, raw, expected):
    if raw is None:
        monkeypatch.delenv("MURINDI_DEPLOYMENT_MODE", raising=False)
    else:
        monkeypatch.setenv("MURINDI_DEPLOYMENT_MODE", raw)
...
def test_saas_mode_allows_T_scope_paper_only(
    ...
    monkeypatch.setenv("MURINDI_DEPLOYMENT_MODE", "saas")
...
def test_saas_mode_live_T_requires_ack(
    ...
    monkeypatch.setenv("MURINDI_DEPLOYMENT_MODE", "hosted")
