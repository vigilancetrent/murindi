...
`/addons/murindi/indicator/getIndicators`.
...
def _validate_indicator_code_internal(code: str, user_params: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if not has_four_way:
        return {
            "success": False,
            "msg": (
                "Missing execution columns. New Murindi indicator scripts must define "
            ...
            "error_type": "MissingExecutionColumns",
            "details": None,
            "plots_count": len(plots),
            "signals_count": len(signals),
            "hints": hints,
...
def ai_generate():
    ...
    # Murindi indicator IDE: chart render + backtest; must pass server verify-code + safe_exec rules.
    SYSTEM_PROMPT = """# Role

You write production-ready **Murindi** indicator scripts: Python that runs in the Indicator IDE, renders on the K-line chart, and drives **backtest entries/exits** via boolean signals. Code must be syntactically valid, safe for the host sandbox, and match the exact I/O contract below.
...
"""
    ...
    def _generate_code_via_llm() -> str:
        ...
        if existing:
            user_prompt = (
                "# Existing Murindi indicator code (migrate it to the four-way execution contract):\n\n```python\n"
                + existing.strip()
                + "\n```\n\n# Change request:\n\n"
                + prompt
                + "\n\nReturn one full replacement script: same Murindi rules (my_indicator_name/description, df = df.copy(), declared @param values must be read via params.get(...), four-way execution columns, output dict, list lengths == len(df)). "
    ...
    def _repair_code_via_llm(bad_code: str, validation: Dict[str, Any]) -> str:
        ...
        repair_prompt = (
            "You produced Murindi indicator code that failed automatic validation. "
            ...
            + "\n```\n\n"
            "# Repair requirements\n"
            "- Keep Murindi indicator contract intact.\n"
