...
Keep manual testing on **1** and automation on **7** (or any other unused id). If you still see drops on Windows, ensure `ib_insync` runs with asyncio patched (Murindi calls `util.patchAsyncio()` at app startup).
