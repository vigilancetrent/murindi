...
# ============================================================
#
# Murindi's webhook channel was originally generic: it POSTed our
...
def _build_webhook_text(payload: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if explicit_title or explicit_msg:
        return (explicit_title or 'Murindi'), (explicit_msg or '')
    ...
    title = ' · '.join(title_bits) if title_bits else 'Murindi 信号'
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        URL hosts recognised automatically:
          ...
          - Slack       hooks.slack.com/services/...
          - Generic     everything else (keeps Murindi's own schema)
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "Murindi/1.0 (+https://www.murindi.com)",
        }
        ...
        if signing_secret and dialect == 'feishu':
            ...
            # Original Murindi contract: body-bound HMAC-SHA256 hex
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "Murindi 通知测试" if zh else "Murindi notification test"
        plain = (
            "这是一条来自 Murindi 个人中心「通知设置」的测试消息。若您收到本条消息，说明该渠道配置正确。"
            if zh
            else "This is a test message from Murindi profile notification settings. "
