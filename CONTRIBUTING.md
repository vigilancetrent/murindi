# Contributing to Murindi

Thanks for your interest in contributing to **Murindi**.

Murindi is a **local-first, private AI-driven quantitative trading workspace**.
...
Contributing to Murindi means creating **public, verifiable work**.
...
The web UI source lives in the separate private **Murindi-Vue** repo, which publishes `ghcr.io/vigilancetrent/murindi-frontend` to GHCR on every `v*` tag — both Compose files pull that image directly.
...
The SPA lives in the private **Murindi-Vue** repo. Tagging a release there (`git tag vX.Y.Z && git push --tags`) triggers `.github/workflows/release-frontend.yml`, which builds a multi-arch image and pushes it to `ghcr.io/vigilancetrent/murindi-frontend`. No frontend artefacts are committed here — pin the consumed tag via `IMAGE_TAG` (or `FRONTEND_TAG` for a per-side override) in a root-level `.env`.

For local iteration without publishing, clone the Vue repo into `./Murindi-Vue/` (gitignored) and run `docker compose -f docker-compose.yml -f docker-compose.build.yml up --build` — see **DEVELOPMENT.md → Building frontend from local source**.
...
Murindi uses **OpenAPI 3** as the HTTP contract SSOT.
...
Murindi may explore incentive or alignment mechanisms in the future.
