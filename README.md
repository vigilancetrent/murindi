<div align="center">
  <a href="https://github.com/vigilancetrent/Murindi">
    <img src="docs/screenshots/logo.jpg" alt="Murindi Logo" width="220" height="220">
  </a>
  ...
  <div align="center" style="max-width: 680px; margin: 1.25rem auto 0; padding: 20px 22px 22px; border: 1px solid #d1d9e0; border-radius: 16px;">
    ...
    <p style="margin: 0 0 18px; padding-bottom: 16px; border-bottom: 1px solid #eaeef2; line-height: 2;">
      ...
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
      <a href="https://www.murindi.com"><strong>Website</strong></a>
      <span style="color: #d8dee4;"> &nbsp;·&nbsp; </span>
    </p>
  ...
  <p style="margin-top: 1.45rem; margin-bottom: 10px;">
    ...
    <img src="https://img.shields.io/github/forks/vigilancetrent/Murindi?style=flat-square&logo=github&label=Forks" alt="Forks">
  </p>
  <p style="margin: 10px 0 12px;">
  </p>
  <p style="margin: 12px 0 10px;">
    <a href="https://oosmetrics.com/repo/vigilancetrent/Murindi"><img src="https://api.oosmetrics.com/api/v1/badge/achievement/4991ab54-52d2-46d4-a03a-67b47b61ae4b.svg" alt="oosmetrics — Top 7 in Training by acceleration (2026-04-25)"></a>
  </p>
...
[Try in 2 minutes](#try-in-2-minutes) · [Why Murindi](#why-murindi) · [Safety model](#safety-model) · [Technical highlights](#technical-highlights) · [Repositories](#related-repositories) · [AI agents & MCP](#use-it-from-an-ai-agent-cursor--claude-code--codex--mcp) · [Overview](#product-overview) · [Features](#features-at-a-glance) · [Visual tour](#visual-tour) · [Architecture](#architecture) · [Install](#installation--first-time-setup-docker-compose) · [Docs](#documentation) · [FAQ](#faq) · [License](#license-and-commercial-terms)
...
Installs to `~/murindi` by default (override: `… | bash -s -- /opt/murindi`). Re-run the same command to pull latest images and restart.

Then open **`http://localhost:8888`**, sign in with **`murindi` / `123456`**, and **change the default admin password**.

<details>
<summary><b>Windows, manual clone, or mirror troubleshooting</b></summary>

**Windows (PowerShell)** — folder name after clone is **`Murindi`**:
...
```bash
git clone https://github.com/vigilancetrent/Murindi.git && cd Murindi && cp backend_api_python/env.example backend_api_python/.env && chmod +x scripts/generate-secret-key.sh && ./scripts/generate-secret-key.sh && docker compose pull && docker compose up -d
```
...
Murindi is a **self-hosted, local-first** quantitative infrastructure layer — not a chatbot with a buy button. It unifies **multi-LLM research**, **Python-native strategy engines**, **server-side backtesting**, and **multi-broker live execution** (10+ crypto venues, IBKR, MT5, Alpaca) in one production-grade stack you fully control.
...
- **Exchange keys stay inside the user's own deployment** — not held by Murindi SaaS operators on self-hosted installs.
- **Every agent call is audit-logged** — append-only audit trail for automation and compliance review.
- **Murindi does not provide investment advice** — software for lawful research and execution only; you are responsible for compliance and risk.
...
<div align="center">
  <img src="docs/screenshots/ezgif.com-animated-gif-maker.gif" alt="Murindi quick demo: install, sign in, charting, AI analysis, and strategy workflow" width="920" style="border-radius: 12px; border: 1px solid #eaeef2;">
  <p><sub><em>From zero to running stack — charting, AI research, and strategy workflow in minutes.</em></sub></p>
</div>

<div align="center">
  <img src="docs/screenshots/architecture.png" alt="Murindi system architecture: Data Sources → Indicator / Signal / Strategy / Backtesting / AI Analysis layers → Execution, with the closed-loop quant workflow (Idea → Indicator → Strategy → Backtest → Optimize → Execute → Monitor)" width="960">
...
| **Agent-native** | First-class **Agent Gateway** (`/api/agent/v1`) + **[`murindi-mcp`](https://pypi.org/project/murindi-mcp/)** on PyPI — Cursor, Claude Code, and Codex can read markets, run backtests, and trade (paper by default) with full audit logs. |
...
|------------|------------|
| **[Murindi](https://github.com/vigilancetrent/Murindi)** (this repo) | Backend (Flask/Python), Compose stack, docs |
| **[Murindi-Vue](https://github.com/vigilancetrent/Murindi-Vue)** | **Web frontend source** (Vue) — tagging `v*` publishes `ghcr.io/vigilancetrent/murindi-frontend` automatically |
| **[Murindi-Mobile](https://github.com/vigilancetrent/Murindi-Mobile)** | **Open-source mobile client** — pairs with your self-hosted or SaaS backend |

**Note:** Node.js is only required if you build the web UI from **Murindi-Vue**; the default Docker quick start pulls the published image and does not need it.

## Use it from an AI agent (Cursor / Claude Code / Codex / MCP)

Murindi ships an **Agent Gateway** at `/api/agent/v1` plus a small **MCP server** ([`murindi-mcp`](https://pypi.org/project/murindi-mcp/) on PyPI) that wraps it as Model Context Protocol tools. Issue a token once and your AI client can read markets, manage strategies, run backtests, and (paper-only by default) place trades — without ever seeing your exchange keys or your admin JWT.
...
```json
{ "mcpServers": { "murindi": {
  "command": "uvx", "args": ["murindi-mcp"],
  "env": { "MURINDI_BASE_URL": "http://localhost:8888",
           "MURINDI_AGENT_TOKEN": "qd_agent_xxxxxxxx" }
} } }
```
...
**Deploy surfaces:** one-line `install.sh`, zero-repo GHCR Compose, full-repo Compose (local backend build), AWS Marketplace AMI, and SaaS at [ai.murindi.com](https://ai.murindi.com) for trials.
...
This section mirrors a typical “local deploy” path: **prepare the host → obtain the code → configure secrets → start the stack → verify → harden → optionally wire AI**. Node.js is **not** required: the `frontend` service pulls `ghcr.io/vigilancetrent/murindi-frontend` directly, so Nginx serves the SPA without any local build step.
...
- **`frontend`** — pulls `ghcr.io/vigilancetrent/murindi-frontend:latest` (no local Vue tree required).
- **`backend`** — built from `./backend_api_python` on first start if no local image exists yet.
- For UI development from Vue source, clone **Murindi-Vue** into `./Murindi-Vue/` and add `-f docker-compose.build.yml` to the command (see *Build the frontend from Vue source* below).
...
# BACKEND_IMAGE=ghcr.io/<your-fork>/murindi-backend     # optional, for forks
# FRONTEND_IMAGE=ghcr.io/<your-fork>/murindi-frontend
```

Tag resolution: `BACKEND_TAG` / `FRONTEND_TAG` → `IMAGE_TAG` → compose default (`latest`). Without a root `.env`, both compose files pull `ghcr.io/vigilancetrent/murindi-{backend,frontend}:latest`. Pin a specific release by setting `IMAGE_TAG` (lockstep) or `BACKEND_TAG` / `FRONTEND_TAG` (per-side) — see [GitHub Releases](https://github.com/vigilancetrent/Murindi/releases) for available tags.
...
If you have access to the **Murindi-Vue** repo and want to iterate on UI source (theme tweaks, forks, debugging) instead of pulling the published image, clone it into the `./Murindi-Vue/` slot at the repo root (gitignored) and let Compose build from there:

```bash
git clone https://github.com/vigilancetrent/Murindi-Vue.git Murindi-Vue
docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build
```

The main `docker-compose.yml` only pulls the GHCR image; the override file `docker-compose.build.yml` adds the local `build:` block. Without the override, `./Murindi-Vue/` does not need to exist. Point `FRONTEND_SRC_PATH=/abs/path/to/Murindi-Vue` if you'd rather keep the source somewhere else, or set `COMPOSE_FILE=docker-compose.yml:docker-compose.build.yml` in a root `.env` to skip the long `-f -f` invocation. The locally built image is tagged the same way as the published one (`FRONTEND_TAG` / `IMAGE_TAG` rules apply), so it slots into the rest of the stack with no further changes.
...
|---------|----------------|
| `Murindi-Vue` not found | You added `-f docker-compose.build.yml` without cloning Vue source. Drop the override (plain `docker compose up -d`) or clone into `./Murindi-Vue/` first. |
...
This is the kind of Python-native strategy logic Murindi is designed for:
...
Murindi supports two main strategy authoring models:
...
├── docker-compose.yml       # Web UI via GHCR; optional ./Murindi-Vue/ for local UI builds
...
| [Agent environment](docs/agent/AGENT_ENVIRONMENT_DESIGN.md) · [AI integration](docs/agent/AI_INTEGRATION_DESIGN.md) · [Quickstart](docs/agent/AGENT_QUICKSTART.md) · [OpenAPI](docs/agent/agent-openapi.json) · [MCP server](mcp_server/README.md) | Coding agents & MCP (`murindi-mcp` on PyPI) |
...
### Is Murindi only for crypto trading?
...
Yes. Murindi supports both dataframe-style `IndicatorStrategy` development and event-driven `ScriptStrategy` development. You can also use AI to generate a starting point and then edit it yourself.

### Is this a research tool or a live trading platform?

It is both. Murindi is built to connect AI research, charting, strategy development, backtesting, quick trade flows, and live execution operations in one system.

### Can I use Murindi commercially?

The backend is licensed under Apache 2.0. The **web** frontend source ([Murindi-Vue](https://github.com/vigilancetrent/Murindi-Vue)) uses a separate source-available license—review both and contact the project for commercial frontend authorization if needed. The **[mobile app repo](https://github.com/vigilancetrent/Murindi-Mobile)** is open source under its own license (see that repository).

### Is there a mobile app?

Yes—see **[Murindi-Mobile](https://github.com/vigilancetrent/Murindi-Mobile)** (open source). It connects to the same backend you self-host or to SaaS.
...
|----------|-------------|
| Binance | [Register](https://www.bsmkweb.cc/register?ref=MURINDI) |
| Bitget | [Register](https://partner.hdmune.cn/bg/7r4xz8kd) |
| Bybit | [Register](https://partner.bybit.com/b/DINGER) |
| OKX | [Register](https://www.xqmnobxky.com/join/MURINDI) |
...
- The frontend source code is available separately at [Murindi Frontend](https://github.com/vigilancetrent/Murindi-Vue) under the **Murindi Frontend Source-Available License v1.0**.
...
- [Report Bugs / Request Features](https://github.com/vigilancetrent/Murindi/issues)
- Email: [support@murindi.com](mailto:support@murindi.com)
...
Murindi stands on top of a strong open-source ecosystem. Special thanks to projects such as:
