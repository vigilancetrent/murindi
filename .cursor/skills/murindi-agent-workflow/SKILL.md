...
2. **`docs/agent/AI_INTEGRATION_DESIGN.md`** — How external AI agents consume Murindi as a product (Agent Gateway, scopes, MCP, trading safety). Read this **before** adding any new endpoint or tool that an AI agent might call.
...
- MCP: `mcp_server/` is a thin Python wrapper over R + W + B endpoints (no
  trading), with three transports selected by `MURINDI_MCP_TRANSPORT`: `stdio` (default,
  desktop IDEs), `sse`, and `streamable-http` (cloud agents / remote IDEs;
  also bind `MURINDI_MCP_HOST` / `MURINDI_MCP_PORT`). Add new tools
  there only after exposing the underlying capability via REST.
- Admin UI: the Vue project at `Murindi-Vue-src/` ships **Profile → My Agent Token**
